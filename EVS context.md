...
- [ ] Nexus: обдумать per-target флаг «WAN/direct» — сейчас при включённом релее он имеет приоритет и WAN-цель (DDNS) пойдёт через релей, что не сработает (релей бродкастит имя в свою LAN). Пока документировано: WAN-режим рассчитан на прямую отправку (релей выключен).
