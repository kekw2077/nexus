...
- `GET /alert-config?device=ID` → `{cpu,ram,disk,temperature,ntfyTopic,localOverride}` — эффективные пороги устройства + признак оверрайда + его топик
...
- [ ] Nexus: пересобрать APK с накопленными правками (прокрутка формы, имя Nexus, per-device) — когда пользователь скажет
...
- [ ] Nexus: проверить на реальном телефоне WebSocket/RMS/HTTPS-релей/Nextcloud/push после деплоя сервера (эта машина без Android SDK)
