...
- **Текущее состояние:** иконка Hub · Status собрана под адаптивную схему Android (`flutter_launcher_icons` подключён и прогнан: `assets/icon/icon.png` + `icon_background.png` + `icon_foreground.png`, safe-zone scale 0.88); версия приложения зафиксирована как **1.0.0** (`pubspec.yaml`, `version: 1.0.0+1`). Заодно найдены и исправлены два бага, не связанных с иконкой: `HostMetrics` не мог быть `const` (статический `_epoch: DateTime` не константа — `DateTime` в Dart вообще не имеет const-конструкторов), это ломало компиляцию (`flutter analyze` падал с `invalid_constant`); `formatBytes` печатал `16.0 ГБ` вместо `16 ГБ` для целых значений — тест падал. Оба исправлены, `flutter analyze` и `flutter test` теперь чистые.
- **Важное ограничение окружения:** на машине, где идёт разработка (Windows, эта сессия), **не установлен Android SDK** и **Visual Studio** (`flutter doctor` — оба toolchain'а красные) — здесь нельзя собрать APK или запустить приложение на Windows-десктопе для проверки. Полноценная сборка/тест на устройстве возможны только на машине с Android Studio (см. README) или на телефоне пользователя.
...
- **Следующий шаг:** собрать APK на машине с Android SDK и проверить иконку/приложение на реальном телефоне; далее — реальный WebSocket в `evs_controller.dart` / эндпоинт под алерты / RMS в `waveform.dart` / механизм обновления.
- **Открытые вопросы:** финализировать переименование `evs_remote` → `nexus`; определить механизм обновления Nexus.
...
HTTP-агент на каждой отслеживаемой машине. Токен — из `/etc/pc-agent.env`, вводится в карточке компьютера и в настройках ретранслятора.
...
### Заглушки / TODO (помечены в коде)

- **EVS** (`evs_controller.dart`) — подключение имитируется; `connect()` ждёт реального WebSocket и функции подключения телефона в десктопном EVS
- **Метрики** — реальный `fetch` готов, работает сразу с запущенным агентом
- **Визуализатор голоса** (`waveform.dart`) — амплитуда синтетическая, `_sample()` заменить на RMS из микрофонного потока

> Статус репозитория: 1 коммит, релизов нет.
...
- [ ] Nexus: подцепить реальный WebSocket в `evs_controller.dart`
- [ ] Nexus: эндпоинт/поток под алерты о проблемах с сервера
- [ ] Nexus: заменить синтетику в `waveform.dart` на RMS с микрофона
- [x] Nexus: собрать иконку Hub · Status под адаптивную схему Android + `flutter_launcher_icons` — сделано 2026-07-11
- [ ] Nexus: собрать APK и проверить иконку/приложение на реальном телефоне (эта машина без Android SDK — см. «На чём остановились»)
