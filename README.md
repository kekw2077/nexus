...
## Что пока заглушки (помечено `TODO` в коде)

- **EVS** (`evs_controller.dart`) — подключение имитируется; на месте `connect()`
  оставлен комментарий под реальный WebSocket. Ждёт функции подключения телефона
  в десктопной версии EVS.
- **Метрики** — реальный `fetch` уже есть; синтетики нет. Работает сразу, как
  только запустите агент.
- **Визуализатор голоса** (`waveform.dart`) — амплитуда синтетическая, метод
  `_sample()` заменяется на RMS из микрофонного потока без изменений вёрстки.
